...
async function registrarHistorico(
  ...
  await prisma.historicoAcao.create({
    data: { estagiarioId, usuarioId, acao, detalhes },
  });
}
...
export async function criarEstagiario(raw: unknown) {
  ...
  if (!vaga || !vaga.ativa) throw new Error("Vaga indisponível");

  const jaOcupada = await prisma.estagiario.findFirst({
    where: { vagaId: data.vagaId, status: { in: ["ATIVO", "AGUARDANDO_CIDE"] } },
  });
  if (jaOcupada) throw new Error("Vaga já está ocupada");
  ...
  await registrarHistorico(
    ...
    "CADASTRO",
    `Estagiário cadastrado na vaga ${vaga.codigo}`
  );
...
export async function ativarEstagiario(estagiarioId: string) {
  ...
  await registrarHistorico(
    ...
    (session.user as { id: string }).id,
    "ATIVACAO",
    "Contratação finalizada — estagiário ativado"
  );
...
export async function buscarEstagiarios(filtros: {
  secretaria?: string;
  status?: string;
  tipo?: string;
  nivel?: string;
  ...
  const estagiarios = await prisma.estagiario.findMany({
    where: {
      ...where,
      ...(filtros.secretaria ? { vaga: { secretaria: filtros.secretaria } } : {}),
    },
