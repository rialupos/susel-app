...
const navItems = [
  ...
  { href: "/estagiarios", icon: Users, label: "Estagiarios" },
  { href: "/vagas", icon: Briefcase, label: "Vagas" },
  { href: "/recesso", icon: Palmtree, label: "Recesso" },
  { href: "/renovacao", icon: RefreshCw, label: "Renovacao" },
    ...
    <aside className="w-64 min-h-screen bg-primary-dark flex flex-col">
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        {navItems.map((item) => {
          ...
          const showBadge = alertCount > 0 && (item.href === "/dashboard" || item.href === "/renovacao");
          return (
            <Link key={item.href} href={item.href} className={cn("flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors", active ? "bg-primary text-white" : "text-blue-200 hover:bg-primary hover:text-white")}>
      ...
      <div className="px-3 py-4 border-t border-primary">
        <button onClick={() => signOut({ callbackUrl: "/login" })} className="flex items-center gap-3 w-full px-3 py-2.5 rounded-lg text-sm font-medium text-blue-300 hover:bg-primary hover:text-white transition-colors">
